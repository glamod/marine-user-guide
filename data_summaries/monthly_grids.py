...
def main():
    ...
    for dt in rrule.rrule(rrule.MONTHLY, dtstart=start, until=stop):
        ...
        if table != 'header':
            if config[table]['average_by_vecors'] == True:
                logging.info('Calculating vecor mean for {}'.format(table))
                cdm_table_ym['sin_obs']=np.sin(np.deg2rad(cdm_table_ym['observation_value']))
                cdm_table_ym['cos_obs']=np.cos(np.deg2rad(cdm_table_ym['observation_value']))
                sin_mean_arr = canvas.points(cdm_table_ym,'longitude','latitude',ds.mean('sin_obs')).assign_coords(time=dt).rename('mean')
                cos_mean_arr = canvas.points(cdm_table_ym,'longitude','latitude',ds.mean('cos_obs')).assign_coords(time=dt).rename('mean')
            ...
            nreports_buoys_arr = canvas.points(cdm_table_ym.loc[cdm_table_ym['platform_type']==5],'longitude','latitude',ds.count(count_param)).\
                assign_coords(time=dt).rename('counts')
            nreports_buoys_list.append(nreports_buoys_arr)
            nreports_ships_arr = canvas.points(cdm_table_ym.loc[cdm_table_ym['platform_type']!=5],'longitude','latitude',ds.count(count_param)).\
                assign_coords(time=dt).rename('counts')
    ...
    if table != 'header':
        ...
        nreports_buoys_agg = xr.concat(nreports_buoys_list,dim = 'time')
        nreports_ships_agg = xr.concat(nreports_ships_list,dim = 'time')
    ...
    if table != 'header':
        ...
        dims_counts = ['latitude','longitude','counts']
        encodings_counts = { k:v for k,v in properties.NC_ENCODINGS.items() if k in dims_counts }
        out_file = os.path.join(config['dir_out'],'-'.join([table,'reports_buoys_grid_ts',config['id_out'] + '.nc']))
        nreports_buoys_agg.encoding = encodings_counts
        nreports_buoys_agg.to_netcdf(out_file,encoding = encodings_counts,mode='w')

        dims_counts = ['latitude','longitude','counts']
        encodings_counts = { k:v for k,v in properties.NC_ENCODINGS.items() if k in dims_counts }
        out_file = os.path.join(config['dir_out'],'-'.join([table,'reports_ships_grid_ts',config['id_out'] + '.nc']))
        nreports_ships_agg.encoding = encodings_counts
        nreports_ships_agg.to_netcdf(out_file,encoding = encodings_counts,mode='w')
